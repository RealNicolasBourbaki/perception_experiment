...
class StartPage:
    ...
    def create_a_csv(filename):
        header_list = ['Stimuli_left', 'Stimuli_right', 'Respond', 'Start_Time', 'Respond_Time', 'Block',
                       'Participant_Number', 'Date', 'Handedness', 'Age', 'Gender', 'Native_Language',
                       'Second_Languages']
...
class SecondPage:
    ...
    def second_screen(self):
        with open(test_path, 'r', encoding='utf8') as input_file:
            ...
            for line in reader:
                self.stimuli_list.append(line[:2])
        self.make_form()
        self.root.mainloop()
    ...
    def play(self):
        try:
            ...
            self.text2.config(text=self.stimuli_list[self.count][0]+'       '+self.stimuli_list[self.count][1],
                              font=('Arial', 17))
    ...
    def make_form(self):
        instruction = 'You are about to see two words on the screen each time, ' \
                      'and you need to decide which one of them is more positive. '
        instruction2 = 'Please press \'D\' if the left one is more positive. ' \
                       'Press \'K\' if the right one is more positive.'
        instruction3 = 'Please hit the space bar to start a training round.'
        s_label = ttk.Style()
        s_label.configure('my.TLabel', font=('Helvetica', 16), background=color)
        row = ttk.Frame(self.root, style='my.TFrame')
...
class ThirdPage:
    ...
    def write_csv(self, respond):
        ...
        with open(output_data_path + filename, 'a', encoding='utf8')as out:
            ...
            csv_writer.writerow([self.stimuli_list[self.count-1][0], self.stimuli_list[self.count-1][1], respond,
...
if __name__ == '__main__':
    ...
    #home_path = '/home/nianheng/Documents/hiwi/01januar/fabian/Tense_Identificantion_Test/' \
                # 'recording_morphology_experiment/00results/ui/decision_exp/'
